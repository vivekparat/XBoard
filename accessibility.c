...
#ifdef USING_SPEECH_DISPATCHER_API
		#include <libspeechd.h>
		SPDConnection *spd_connection;
#elif  USING_ESPEAK_API
		#include <glib.h>
		#include <speak_lib.h>
#endif
...
#ifdef USING_ESPEAK_API
gpointer espeak_say(void *text)
{
	int Size;
	char received[8000];
	sprintf(received,"%s",(char *)text);
	espeak_POSITION_TYPE position_type = POS_SENTENCE;
	espeak_Cancel();	 
	Size = strlen(received)+1;
	espeak_Synth(received, Size, 0, position_type, 0,	espeakCHARS_AUTO,0, NULL);
	espeak_Synchronize(); 
	return (NULL);
}
#endif
...
void TTS_say(char *buf)
{
		...
		spd_say(spd_connection, 1, buf);
#elif USING_ESPEAK_API
		char tts_buf[8000];			//Kludge - buf will be Nulled by set_accessible_description
		sprintf(tts_buf,"%s",buf);	//before thread starts - Nalin-x-GNU					
		g_thread_new("Espeak",espeak_say, tts_buf);
#else
		char tts_buf[8000];
